import * as e from "react";
import { useForm as S } from "../../contexts/form/form.es.mjs";
import { useEventListener as _ } from "../../hooks/use-event-listener.es.mjs";
import { PayButton as h, SvgIcon as P } from "./ach.styles.es.mjs";
import { transformPlaidEventName as O } from "./ach.utils.es.mjs";
function M({
  accountHolderName: E,
  intent: w,
  total: y,
  redirectURI: b,
  transactionId: v,
  callbacks: a,
  buttonProps: g,
  children: c,
  svgProps: k
}) {
  const [r, z] = e.useState(() => {
  }), [R, f] = e.useState(!1), { cardTokenizeResponseReceived: x, payments: u } = S(), s = e.useRef(null), C = async (n) => {
    if (n.stopPropagation(), !r) {
      console.warn("ACH button was clicked, but no ACH instance was found.");
      return;
    }
    try {
      await r.tokenize({
        accountHolderName: E,
        intent: w,
        total: y
      });
    } catch (t) {
      console.error(t);
    }
  }, d = async (n) => {
    f(!0);
    try {
      const { tokenResult: t } = n.detail;
      if (t && t.status === "OK")
        return await x(t);
      let i = `Tokenization failed with status: ${t?.status}`;
      if (t?.errors)
        throw i += ` and errors: ${JSON.stringify(t?.errors)}`, new Error(i);
    } catch (t) {
      console.error(t);
    } finally {
      f(!1);
    }
  };
  if (e.useEffect(() => (r?.addEventListener("ontokenization", d), () => {
    r?.removeEventListener("ontokenization", d);
  }), [r]), e.useEffect(() => {
    const n = new AbortController(), { signal: t } = n;
    return (async (l) => {
      const L = await u?.ach({
        redirectURI: b,
        transactionId: v
      }).then((p) => {
        if (!l?.aborted)
          return z(p), p;
      });
      l.aborted && await L?.destroy();
    })(t), () => {
      n.abort();
    };
  }, [u]), a)
    for (const n of Object.keys(a))
      r?.addEventListener(
        O(n),
        a[n]
      );
  _({
    listener: C,
    type: "click",
    element: s,
    options: {
      passive: !0
    }
  });
  const { isLoading: A, ...m } = g ?? {}, o = A || !r || R;
  return c ? /* @__PURE__ */ e.createElement(h, { ...m, "aria-disabled": o, disabled: o, ref: s, type: "button" }, c) : /* @__PURE__ */ e.createElement(h, { ...m, "aria-disabled": o, disabled: o, ref: s, type: "button" }, /* @__PURE__ */ e.createElement(
    P,
    {
      fill: "none",
      height: "1em",
      viewBox: "0 0 36 24",
      width: "1em",
      xmlns: "http://www.w3.org/2000/svg",
      ...k
    },
    /* @__PURE__ */ e.createElement("rect", { fill: "url(#prefix__paint0_linear)", height: 24, rx: 4, width: 36 }),
    /* @__PURE__ */ e.createElement(
      "path",
      {
        clipRule: "evenodd",
        d: "M18.509 6.16a.89.89 0 00-1.018 0l-6.124 4.285a.848.848 0 00-.325.96.873.873 0 00.833.595h1.75v4.286h-1.75a.866.866 0 00-.875.857c0 .473.392.857.875.857h12.25a.866.866 0 00.875-.857.866.866 0 00-.875-.857h-1.75V12h1.75c.38 0 .717-.24.833-.596a.848.848 0 00-.324-.959L18.509 6.16zm2.116 10.126V12h-5.25v4.286h5.25zM18 7.91l3.395 2.376h-6.79L18 7.91z",
        fill: "#fff",
        fillRule: "evenodd"
      }
    ),
    /* @__PURE__ */ e.createElement("defs", null, /* @__PURE__ */ e.createElement("linearGradient", { gradientUnits: "userSpaceOnUse", id: "prefix__paint0_linear", x1: 36, x2: 0, y1: 12, y2: 12 }, /* @__PURE__ */ e.createElement("stop", { stopColor: "#01D09E" }), /* @__PURE__ */ e.createElement("stop", { offset: 1, stopColor: "#03E4AE" })))
  ), /* @__PURE__ */ e.createElement("span", null, "Pay with Direct debit (ACH)"));
}
export {
  M as Ach,
  M as default
};
//# sourceMappingURL=ach.es.mjs.map
